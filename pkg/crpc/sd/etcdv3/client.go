...
func (c *client) Register(s *sd.Service) error {
	var err error
	...
	if s.TTL == nil {
		s.TTL = sd.NewTTLOption(3*time.Second, 10*time.Second)
	}
	...
	manager, _ := endpoints.NewManager(c.cli, s.Key)
	err = manager.AddEndpoint(
		c.ctx,
		fmt.Sprintf("%s/%s", s.Key, s.Value),
		endpoints.Endpoint{Addr: s.Value},
	...
	// this will keep the key alive 'forever' or until we revoke it or the context is canceled
...
func (c *client) Deregister(s *sd.Service) error {
	defer c.close()

	if s.Key == "" {
		return ErrNoKey
	}
	key := fmt.Sprintf("%s/%s", s.Key, s.Value)
	if _, err := c.cli.Delete(c.ctx, key, clientv3.WithIgnoreLease()); err != nil {
		slog.Error("Deregister", "err", err)
		return err
	}

	slog.Info("Deregister", "k", key)
	return nil
}
